...
class Classifier {
 ...
 const string& label_file*/);
	...
	void getFeature(std::string name,  std::string outputFile, std::string featureLayer);
...
void Classifier::getFeature(std::string file, std::string outputFile, std::string featureLayer) {
	...
	string blob_name = featureLayer;
	CHECK(net_ -> has_blob(blob_name)) << "Unknown feature blob name " << blob_name << " in the network";

	feature_blob = net_->blob_by_name(blob_name);

	int channel = feature_blob->channels();
	std::ofstream fout;
	fout.open(outputFile.c_str(), std::ofstream::out | std::ofstream::app);
	// std::cout << channel << std::endl;
	fout << file << ",";
	for (int i = 0; i < channel; i++) {
		fout << feature_blob->data_at(0, i, 0, 0);
		if (i != channel - 1)
			fout << ",";
	}
	fout << "\n";
...
int main(int argc, char** argv) {
	...
	string layerName = argv[5];
	string file = argv[6];

	std::cout << "---------- Extract Features from " << file << " ----------" << std::endl;

	// cv::Mat img = cv::imread(file, -1);
	// CHECK(!img.empty()) << "Unable to decode image " << file;
	classifier.getFeature(file, outputFile, layerName);
