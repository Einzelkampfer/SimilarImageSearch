...
class Classifier {
 ...
 const string& label_file*/);
	...
	void getFeature(const cv::Mat& img);
...
void Classifier::getFeature(const cv::Mat& img) {
	Blob<float>* input_layer = net_->input_blobs()[0];
	...
	string blob_name = "loss3/classifier";
	...
	std::ofstream fout;
	std::string outputFile = "build/examples/cpp_classification/output.txt";
	fout.open(outputFile.c_str());
	std::cout << channel << std::endl;
	for (int i = 0; i < channel; i++) {
		fout << feature_blob->data_at(0, i, 0, 0) << std::endl;
	}
	fout.close();
}
